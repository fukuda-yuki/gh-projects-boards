...
Recognized Windows credential-store (`keyring`) authentication permits guarded adapter writes. Plaintext or unknown storage remains diagnosable without permitting writes.

Commands copied into your terminal use that terminal's environment; remove token overrides there when updating stored authentication.
...
Use relevant focused tests during implementation. UI changes also need [UI integration](tests/README.md#ui-integration) for presentation/binding behavior. The current solution has no dedicated UI integration host/runner; Core tests and external-driver E2E must not be reported as that missing coverage. Add the smallest task-relevant UI test infrastructure or explicitly record the gap.

### Supplementary boundary checks

Select these for the changed boundary or an explicit acceptance need; this is a command reference, not a checklist to run in full for every change. Full regression remains available when warranted. Use the documented E2E filters for a scoped run and report its actual scope.
...
CI runs deterministic tests and discovers desktop tests without launching UI. It does not establish UI integration, desktop, IME, live-system or company GHEC + EMU acceptance. Current execution evidence and incomplete feature scope belong to the owning Issues, not this README.
