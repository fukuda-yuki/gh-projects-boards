...
Preparation works in an explicitly selected saved profile without authentication. The new-row count and missing title/destination/option notices are separate from existing-field differences. New rows are **not yet remotely creatable** and the Apply dialogs explicitly exclude them. Save, switch Projects, refresh and restart retain identities, destinations and pending text. The authoritative version 4 checkpoint includes local records and row Undo; versions 1–3 and Apply recovery remain readable. To check recovery, close normally and reopen the same data root/profile/Project; pending text remains uncommitted. Unregister with retention and re-register the same scoped Project to reopen retained rows.
...
Execution history is stored in version 4 of the authoritative profile checkpoint alongside remaining drafts and cache observations. Preserve the entire profile and backups for recovery. Versions 1–3 remain readable. Live product validation is opt-in via `scripts/Test-ApplyLive.ps1`; it creates only a disposable sandbox fixture and independently verifies cleanup.
