...
4. Inspect **反映結果・履歴**, directly beside the Apply command and inside confirmation. It opens after execution unless active IME composition requires retaining editor focus. Per-field outcomes distinguish verified success, failure, unsent work and uncertain results; creation stages retain their separate evidence. A later edit or pending buffer survives acknowledgement of an earlier approved value. Unselected rows remain local work. Cancellation stops unsent operations and does not undo completed GitHub updates.
