...
All results are under `TestResults/performance/<run-id>/`; desktop/live runner logs link their additional retained evidence directories. The manifest precedes sampling and records source, workload, environment and policy. Build/binary hashes identify execution sources. `-NoBuild -Executable <path> -SourceRevision <40-character-SHA>` can use an immutable copied synthetic executable. The source revision is caller-declared: retain its original build record, including any common harness/instrumentation overlay, and verify the hashes. The current checkout's HEAD identifies the invoking script, not necessarily that binary.
