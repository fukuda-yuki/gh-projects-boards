...
Source: #10 and the #8/#9 existing-field contracts. Only existing Issue titles and Project-owned single-select values (including explicit clear) are supported. No remote creation/Project addition, Draft conversion, Issue state, other field types, all-Project execution or remote Undo is included.
...
Version 4 uses the existing whole-profile checkpoint, serialized save gate and guarded Undo history. Versions 1–3 migrate without discarding Apply attempts, successful/unresolved outcomes, conflicts or pending work. Older writers cannot overwrite the new schema. Repository existence, creation permission, Issue creation, Project addition and returned remote IDs remain #11.
