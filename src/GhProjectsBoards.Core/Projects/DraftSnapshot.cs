...
internal static class DraftSnapshot
{
    ...
    private static ProjectPlanning Plan(ProjectPlanning plan) => plan with
    {
        ...
        Summary = plan.Summary is { } summary ? summary with
        {
            Allowances = summary.Allowances.ToArray(),
            Baseline = summary.Baseline is { } baseline ? baseline with
            { Calendar = Calendar(baseline.Calendar), People = baseline.People.ToArray(), Tasks = baseline.Tasks.ToArray() } : null
        } : null
    };
    private static PlanningCalendar Calendar(PlanningCalendar calendar) => calendar with
    {
        Holidays = calendar.Holidays with { Dates = calendar.Holidays.Dates.ToArray() },
        Exceptions = calendar.Exceptions.Select(e => e with { Intervals = e.Intervals.ToArray() }).ToArray()
    };
}
