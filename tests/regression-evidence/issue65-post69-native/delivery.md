...
Prepared PR and Issue text and a Git bundle are under `TestResults/issue65-post69-native/handoff/`. Only the new product/evidence commits belong to this review unit. Ignored native traces/symbols are excluded from the bundle. A maintainer can publish the branch with:

```powershell
git push -u origin codex/issue-65-post69-native
```

Use `Refs #65`, not `Closes #65`. Functional guards pass; scroll performance remains failed. No further capture/UAC action is pending for this completed acquisition. Human Fresh/Weekly evaluation may proceed independently on the stable isolated evaluation checkout, without competing with engineering runs. A future native acquisition requires its own precise bounded consent. Summary expansion, later features and automatic main integration remain outside this handoff.
