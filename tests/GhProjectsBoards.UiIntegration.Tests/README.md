...
`SheetContextHostedTests` covers frozen identity/pending caret during horizontal movement, header width/visibility and inline/field filtering with real durable reload, failed header-save retention, and the visible native scrollbar's public RangeValue-to-viewport collaboration. The same native title editor must survive width changes and focused scrolling. Choice-cell tests activate the real button and native option MenuFlyout; the durable option-ID/clear/Undo assertions retain ownership of the behavior, rather than requiring a ComboBox implementation. Ordinary `WorkspaceReconstructionSession` additionally uses actual header resizing and direct header/inline view commands; the existing column/row-view journeys retain full settings-dialog and clipboard/restart coverage.
...
| Undo a removed local row after checkpoint reload without exposing other hidden work | `UndoRowVisibilityHostedTests.cs` extends `HostedTests`: `ReopenedViewUndoRestoresRemovedLocalRowTemporarilyWithoutRevealingOtherHiddenWork` reloads the real checkpoint and invokes public Undo/Reapply controls; the restored identity absent from that projection appends temporarily, with original pending text, prior displayed order and preferences retained; Reapply removes the exception | Core local-row tests own removal/Undo identity and durable recovery; ordinary `LocalRowsSaveFailureAndInterruptionRecoverIdentitiesAndUndo` retains native scrolling, process shutdown and recovery |
| The actual sheet fits its header/footer and reaches the last row without stealing focus | `SheetViewportHostedTests.WorkspaceViewportFitsBetweenHeaderAndFooterAndReachesLastRowWithoutMovingFocus`: real panel at two sizes, transformed native layout bounds, ScrollViewer bottom movement, loaded last-row visibility, retained command focus and unchanged work | External UIA-reported bounds/setter behavior and physical-wheel input require separate ordinary-app observations; hosted geometry is not visual acceptance |
