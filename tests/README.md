...
Local-row tests exercise whole-profile version 1–3 migration (including version 3 successful/unresolved operations, attempts, conflicts and drafts), independent identities/destinations, all-or-nothing append/duplication/removal, mixed-paste Undo after Apply, stale commit guards, failed acknowledgement and retained re-registration. Refresh tests cover complete/partial/failed/cancelled observations and disappearing options without local-row loss. Ordinary-app journeys include offline addition, existing/local continuous editing, removal/Undo, normal restart, save failure, forced interruption and Apply exclusion. Four additional `GridIme` cases type physical Japanese into new and duplicated titles through direct/F2 paths and recover pending text after refresh/restart. Fixture calls for local preparation must contain zero product mutations; the separate existing Apply journey asserts its exact existing-Issue payload.
