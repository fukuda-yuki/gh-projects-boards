...
Contextual planning uses `PlanningAssignmentTests` and `PlanningCellInputTests` for native assignment/weight ambiguity, retained legacy ownership, independent raw-hour inputs, dated cumulative actuals, explicit zero/removal and minute endpoints. `DraftLifetimeTests` exercises detached snapshots, requests arriving during a failed save, and reentrant completion with real isolated storage. These rules do not need duplicate whole-app permutations.

`PlanningHostedTests` drives the actual Project mapping/weight controls, typed/F2/paste Actual context, one confirmed reporting date across rows, independent Remaining editing, multiworker breakdown/cancel, native date/time controls and operation Undo. `TaskDetailsClosureRetainsTheNextActualInputAndItsFocus` supplies the next control input at the modal closing boundary and inspects the current control after the animation: an earlier UIA element can survive after its visible replacement. The ordinary planning journey additionally checks the current Actual cell and keyboard focus after a preceding task-details dialog. Gantt/Boards popup geometry cases verify that input and Apply/Close actions remain in the visible viewport.

For an isolated ordinary-app evaluation, build Release and run `./scripts/Start-PlanningCheck.ps1 -Scenario Weekly` (or `Fresh`, `Load`). It creates a new synthetic root, strips token variables from the child, and records source/binary hashes. `Fresh` has no initialized per-task plan; `Weekly` has Actual 5 and Remaining 4; `Load` has 1,000 tasks / 20 people and explicitly retained legacy ownership. Select the cached profile and P1 without connecting. Resume only the generated root with `-Resume -DataRoot <absolute-path>`; existing data is never replaced. This launcher prepares a working loop for evaluation, not human sign-off or a live GitHub test.

`Test-SustainedInput.ps1` is opt-in native input/scroll diagnostics against a frozen ordinary executable, separate from E2E workflow assertions. It requires exact source and a fresh run ID; `-SeedExecutable` permits the same archived fixture generator on old and new sources. Standard mode has 20 seconds each of title input, NUMBER input, and physical wheel/horizontal motion. Warm mode adds ten unmeasured seconds. IME mode uses 60 seconds of physical composition/conversion/confirmation with an isolated writer-lock failure and explicit save retry. Scroll mode adds a native scrollbar-thumb jump, immediate distant-row edit, horizontal roundtrip and wheel return. Normal close independently verifies the last pending text, original offscreen input, unchanged plan attribution and history. The separate ordinary Gantt journey owns actual restart and Project switching.

`Measure-SustainedInput.ps1` retains the complete native-value latency and rendering/capture-gap distributions, each >=100 ms interval, flush requests versus durable commits, and trace-drain integrity. Native-value readback includes UIA observer overhead; rendering callbacks precede composition. Timestamped GDI frames are independent observations, not interchangeable with either signal. A diagnostic completion alone is neither a timing-threshold pass nor readable scrolling/human acceptance. Keep failed setup attempts and partial traces separately, and disclose setup animation waits outside the workload.
...
`python -m unittest discover -s tests -p test_sustained_desktop.py` checks the existing desktop evaluator offline, including the boundary between consecutive requests, late first responses, wrong content, missing review and missed desktop updates. These synthetic timeline regressions do not launch the app or establish its performance.
