...
`PlanningHostedTests` drives real planning, weekly report, batch Auto, Project settings/preset adoption and unavailable-field comparison controls. `OrdinaryBoardsManualDependencyWeeklyReplanRestartsAndPublishesReviewedProjections` is the representative ordinary-app E2E through isolated fake gh, including exact Manual restart and reviewed publication. `PlanningApplicationLiveTests` separately exercises production reader, workspace, durable drafts, Apply/readback and external DATE reconciliation using two uniquely owned Issues in the authorized sandbox; opt in with `GHPB_RUN_PLANNING_APP_PROOF=1`, `GHPB_LIVE_GH_PATH` and a fresh `GHPB_LIVE_ARTIFACTS` directory. It independently checks owned cleanup and preservation of pre-existing Project data. It is not throughput or human acceptance.
