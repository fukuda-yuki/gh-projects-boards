...
public sealed class ReadyInputTests
{
    ...
    public void NativeEditorKeepsCellAndImeBoundaries(string scenario)
    {
        ...
        finally
        {
            ...
            {
                var forcedCleanup = !process.HasExited;
                if (forcedCleanup)
                {
                    process.Kill(entireProcessTree: true);
                    process.WaitForExit(10000);
                }
                File.WriteAllText(Path.Combine(output, scenario + "-lifetime.json"), JsonSerializer.Serialize(new
                {
                    processId = process.Id, forcedCleanup, exited = process.HasExited,
