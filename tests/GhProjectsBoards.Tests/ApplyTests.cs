...
internal sealed class ApplyTests
{
    ...
    public void CheckpointExplicitlyVersionsExecutionHistory()
    {
        ...
        Assert.That(workspace.Snapshot().Version, Is.EqualTo(3));
    }
    [Test]
    ...
    public async Task OlderProfileMigratesWithRecoveryBackupAndJournal(int version)
    {
        ...
        Assert.That((await store.LoadAsync(w.Scope))!.Version, Is.EqualTo(3)); Assert.That(File.Exists(store.FileFor(w.Scope) + ".bak"), Is.True);
    }
    [Test]
