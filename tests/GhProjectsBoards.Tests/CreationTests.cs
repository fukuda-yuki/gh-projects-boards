...
internal sealed class CreationTests
{
    ...
    public async Task RealisticVersionFourMigratesRowsUndoPendingAndSelectIntent()
    {
        ...
        Assert.That((await store.LoadAsync(w.Scope))!.Version, Is.EqualTo(11)); Assert.That(File.Exists(store.FileFor(w.Scope) + ".bak"), Is.True);
    }
    [Test]
    ...
    public void CheckpointUsesNextExplicitVersion()
        => Assert.That(new EditingWorkspace(new("github.com", 42)).Snapshot().Version, Is.EqualTo(11));
